...
func (r *ProofArtifactRepository) GetChainedProofLayers(ctx context.Context, proofID uuid.UUID) ([]ChainedProofLayer, error) {
	...
	for rows.Next() {
		...
		// Use *[]byte for nullable bytea/jsonb columns to handle NULLs gracefully
		var sourceHash, targetHash *[]byte
		var receiptEntriesRaw *[]byte
		if err := rows.Scan(
			...
			&sourceHash, &targetHash, &receiptEntriesRaw,
		...
		if receiptEntriesRaw != nil {
			l.ReceiptEntries = json.RawMessage(*receiptEntriesRaw)
		}
...
func (r *ProofArtifactRepository) GetTimelineByIntentID(ctx context.Context, intentID string) ([]IntentTimelineEvent, error) {
	...
	query := `
		...
		FROM batch_transactions bt
		JOIN proof_artifacts pa ON bt.intent_id = pa.intent_id
		LEFT JOIN anchor_batches ab ON bt.batch_id = ab.id
		WHERE bt.intent_id = $1
		LIMIT 1`
