...
func TestTransactionLookupsAcceptTheTransactionID(t *testing.T) {
	...
	// proofs_service only reads the lifecycle; the validator writes it.
	lifecycle := NewIntentLifecycleRepository(client)
	if _, err := testDB.ExecContext(ctx, `
		INSERT INTO intent_lifecycle (intent_id, accum_tx_hash, block_height, proof_class, target_chain, status,
			submitted_at, authorized_at, created_at, updated_at)
		VALUES ($1, $2, 1, 'on_demand', 'base-sepolia', 'authorized', NOW(), NOW(), NOW(), NOW())`, intentID, hash); err != nil {
	...
	for name, value := range forms {
		...
		if got, err := artifacts.GetTransactionMetadata(ctx, value, nil); err != nil || got == nil || got.IntentID != intentID {
			t.Errorf("%s: GetTransactionMetadata = %+v, %v", name, got, err)
		}
