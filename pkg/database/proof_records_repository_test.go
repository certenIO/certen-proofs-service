...
func TestBLSAttestationsAndTheirAggregate(t *testing.T) {
	...
	if _, err := testDB.ExecContext(ctx, `
		INSERT INTO aggregated_attestations (
			aggregation_id, cycle_id, scheme, message_hash, aggregated_signature, aggregated_public_key,
			participant_ids, participant_count, total_weight, achieved_weight, threshold_weight, threshold_met,
			threshold_numerator, threshold_denominator, aggregated_at
		) VALUES ($1, $2, 'bls12-381', $3, $4, $5, '["v1","v2"]', 2, 3, 2, 3, FALSE, 2, 3, NOW())`,
		cycleAggID, cycleID, message, hash32("cycle-sig"), hash32("cycle-pk")); err != nil {
...
func TestProofRequestLifecycle(t *testing.T) {
	requireTestDB(t)
	ctx := context.Background()
	requests := NewRequestRepository(&Client{db: testDB})
	artifact := newTestArtifact(t, ctx)
...
func TestBatchTransactionsWithoutProofsAreReadable(t *testing.T) {
	...
	batches := NewBatchRepository(&Client{db: testDB})
